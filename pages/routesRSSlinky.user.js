  ...
  const urlRegex= /Matches for query[\s]*\((.*)\)/ig;
    ...
    for (var cand = null, i = 0; (cand = candidates.snapshotItem(i)); i++) {
        if (urlRegex.test(cand.nodeValue)) {
		    ...
		    XMLURL='http://routes.open.ac.uk/routes_xml_rss.php?course='+match[1];
                a.setAttribute("href", XMLURL);
			br=document.createElement("br");
			a.appendChild(br);
		    img=document.createElement("img");
			img.setAttribute("src","http://myopenlibrary.open.ac.uk/images/rss_feed.gif"); img.setAttribute("border", 0); img.setAttribute("alt","XML-RSS feed");
                a.appendChild(img);
